...
class LogBuffer; // forward declaration
...
public:
    ...
    void cut_and_post_pre_only_(double ts_exec, const EpochMeta& meta);
...
public:
    using InferFn = std::function<std::optional<Direction>(
        const std::vector<EEGSample>& epoch,
        const EpochMeta& meta)>;

    void set_infer_fn(InferFn fn);
...
private:
    ...
    //LSL "decide" meta sending
    std::mutex decide_mtx_;
    std::string last_dir_label_;   // e.g. "MOVE_Left" or "ZOOM_ZoomIn"
    int last_trial_id_ = -1;
    double last_decide_ts_ = 0.0;
...
private:
    void log_push_(const std::string& s);
