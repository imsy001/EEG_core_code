...
namespace config {
    inline constexpr double kPreSec = 2.5;
...
namespace eeg {
    ...
    namespace {
        ...
        // Convert epoch (vector of EEGSample) into CH-FIRST float buffer:
        // layout: [c * nt + t]  (so it matches your current run(epoch_ch_first, nch, nt))
        static std::vector<float> epoch_to_ch_first(
            ...
            for (int t = 0; t < tmax; ++t) {
                ...
                // If channels missing, remaining are left as 0
    ...
    const EpochMeta& meta)
{
    ...
    // 1) If user provided an inference function, call it --? always call inference function?
    InferFn fn;
    {
    ...
    std::optional<Direction> pred;

    if (fn) {
        pred = fn(*epoch, meta);  // <-- epoch passed in-memory
    }
    else if (config::kEnableOnlineInference) {
        // fallback (your current dummy inference)
        pred = infer_direction_fast(*epoch);
    }

    // 2) Emit result (LSL or whatever you use)
    if (pred.has_value()) {
        lsl_.send_direction(*pred, meta.trigger_ts);
    }
}
...
//-----------------------inference-------------------
void Controller::set_infer_fn(InferFn fn) {
    std::lock_guard<std::mutex> lk(infer_mtx_);
    infer_fn_ = std::move(fn);
}
